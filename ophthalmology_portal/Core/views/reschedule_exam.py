...
class RescheduleExam(BaseView):
    ...
    def post(self, request: HttpRequest, exam_id, *args, **kwargs):
        ...
        if datetime.datetime.now(
            ...
            return redirect(reverse("request_exam", exam_id))
        exam = ExamModel.objects.get(id=exam_id)
        form = ExamRescheduleMainForm(request.POST, instance=exam)
        if form.is_valid():
            form.save()
            return redirect(f"/exam-history/{exam_id}")
        return redirect(f"/reschedule-exam/{exam_id}")
